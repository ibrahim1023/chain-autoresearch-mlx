...
contract ManualComparatorLedgerActor {
    TokenLedgerManualComparator internal immutable ledger;

    constructor(TokenLedgerManualComparator ledger_) {
        ledger = ledger_;
    }
...
contract TokenLedgerManualComparatorInvariantTest {
    TokenLedgerManualComparator internal ledger;
    ManualComparatorLedgerActor internal actorA;
    ManualComparatorLedgerActor internal actorB;
    ManualComparatorLedgerActor internal actorC;

    function setUp() public {
        ledger = new TokenLedgerManualComparator();
        actorA = new ManualComparatorLedgerActor(ledger);
        actorB = new ManualComparatorLedgerActor(ledger);
        actorC = new ManualComparatorLedgerActor(ledger);
    }
    ...
    function _assertTrackedSupply(uint256 expectedSupply) internal view {
        uint256 trackedBalances = ledger.balanceOf(address(actorA)) + ledger.balanceOf(address(actorB))
            + ledger.balanceOf(address(actorC));
